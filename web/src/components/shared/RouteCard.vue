...
async function initMap() {
  ...
  const center: [number, number] = props.from ? [props.from.lng, props.from.lat] : [113.29, 22.81]
...
async function searchRoute() {
  ...
  try {
    ...
    if (mode === 'transit') {
      ...
      if (!T) {
        error.value = '公交路线服务加载失败'
        loading.value = false
        return
      }
      new T({ map: amap, city: props.from?.name || '', autoFitView: true }).search(
        fromLL,
        toLL,
