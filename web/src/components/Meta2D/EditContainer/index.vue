...
<template>
  ...
  >
    <div ref="editContainerRef" class="code-editor" :style="{ height: editorHeight }"></div>
    <span
      >打开图纸后，执行的初始脚本。 <br />可获取pen和context参数 <br />例如，console.log('pen', 'context');return
      true;</span
    >
  </a-modal>
</template>
...
function computeEditorHeight(value) {
  ...
  return Math.min(window.innerHeight * 0.8, Math.max(minHeight, idealHeight)) + 'px'
}
...
async function ensureEditor(emit, container) {
  ...
  if (!persistentContainer) {
    ...
    persistentContainer.style.cssText = 'position:fixed;top:-9999px;left:0;width:100%;height:80vh;pointer-events:none;'
