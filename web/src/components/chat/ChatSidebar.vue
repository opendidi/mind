...
<template>
  <aside class="chat-sidebar" :class="{ collapsed }">
        ...
        <div class="conv-item" :class="{ active: conv.id === activeConvId }" @click="$emit('select', conv.id)">
          ...
          <div class="conv-meta">
            ...
            <div class="conv-item-actions">
              <a-button type="text" size="small" class="conv-pin" :class="{ pinned: conv.pinned }" @click.stop="$emit('togglePin', conv.id)">
              ...
              <a-dropdown :trigger="['click']" placement="bottomRight">
                ...
                <template #overlay>
                  <a-menu @click="({ key }) => onMenuAction(key as string, conv.id)">
                    ...
                    <a-menu-divider />
                    <a-menu-item key="delete"><span class="menu-delete"><DeleteOutlined /> 删除</span></a-menu-item>
                  </a-menu>
                </template>
              </a-dropdown>
            </div>
    ...
    <div class="sidebar-footer">
      <a-button type="text" danger size="small" block
...
import {
  EditOutlined, MenuFoldOutlined, MenuUnfoldOutlined,
  DeleteOutlined, SearchOutlined, PushpinOutlined, PushpinFilled,
...
.chat-sidebar {
  ...
  background: #f1f5f9; border-right: 1px solid $border;
  ...
  .sidebar-header {
    ...
    .sidebar-logo {
      ...
      text-decoration: none; color: $text; font-weight: 700; font-size: 15px;
    }
  ...
  .sidebar-search {
    ...
    :deep(.ant-input-affix-wrapper) {
      border-radius: 10px; background: #e2e8f0; border: 1px solid transparent;
      padding: 4px 10px; transition: all 0.2s ease;
      .ant-input {
        background: transparent; font-size: 13px; height: 28px; line-height: 28px;
        &::placeholder { color: $text-muted; font-size: 12px; }
      }
      .anticon { color: $text-muted; }
      &:hover { background: #dde4ed; }
      &:focus-within { background: #fff; border-color: $primary; box-shadow: 0 0 0 2px rgba($primary, 0.1); }
    }
  }
  ...
  .sidebar-list {
    ...
    .conv-item {
      ...
      .conv-meta {
        ...
        .conv-time { font-size: 11px; color: $text-muted; }
        .conv-item-actions { display: flex; align-items: center; gap: 0; opacity: 0; transition: opacity 0.15s; }
        .conv-pin {
          font-size: 11px; color: $text-muted; padding: 0; min-width: 22px; height: 22px;
          &:hover { color: $primary; }
          &.pinned { opacity: 1; color: $primary; }
        }
        .conv-more-btn {
          font-size: 14px; color: $text-muted; padding: 0; min-width: 24px; height: 24px;
          &:hover { color: $text; }
        }
      }
      &:hover .conv-item-actions { opacity: 1; }
    }

    .conv-empty { text-align: center; color: $text-muted; font-size: 13px; padding: 24px 0; }
    .conv-more {
      text-align: center; color: $primary; font-size: 12px; padding: 10px;
      cursor: pointer; border-radius: 6px;
      &:hover { background: rgba($primary, 0.06); }
    }
  }

  .sidebar-footer { padding: 8px; border-top: 1px solid $border; }
}
