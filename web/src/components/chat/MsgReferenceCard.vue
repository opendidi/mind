  ...
  <div v-if="references && references.length > 0" class="msg-refs">
    <div class="refs-header" @click="$emit('selectRefs', references)">
      ...
      <span class="refs-label"
        >阅读了 <strong>{{ references.length }}</strong> 个网页</span
      >
      <svg
        class="refs-chevron"
        viewBox="0 0 24 24"
    ...
    <div class="refs-list">
      <template v-for="(ref, idx) in references.slice(0, 3)" :key="idx">
        ...
        <a class="refs-link" :href="ref.url" target="_blank" rel="noopener" :title="ref.title || ref.url" @click.stop>
          {{ idx + 1 }}. {{ ref.title || ref.domain || ref.url }}
        </a>
      </template>
      <span v-if="references.length > 3" class="refs-more"> · 等{{ references.length }}条 </span>
    </div>
  </div>
</template>
