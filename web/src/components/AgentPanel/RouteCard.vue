...
const SERVICE_MAP: Record<string, string> = {
  driving: "AMap.DrivingRoute", walking: "AMap.WalkingRoute",
  riding: "AMap.RidingRoute", transit: "AMap.TransferRoute",
};
...
async function searchRoute() {
  ...
  try {
    if (mode === "driving") {
      const DrivingRoute = (_AMap as any).DrivingRoute;
      if (!DrivingRoute) { error.value = "驾车路线服务加载失败"; loading.value = false; return; }
      const driving = new DrivingRoute({ map: amap, autoFitView: true });
      driving.search(fromLL, toLL, (status: string, result: any) => {
        loading.value = false;
        if (status === "complete" && result.routes?.length) {
          processDrivingRoutes(result.routes);
          renderMarkers(props.from!.lng, props.from!.lat, props.to!.lng, props.to!.lat);
        } else {
          error.value = "驾车路线计算失败，请检查起终点位置";
        }
      });
      return;
    }
    ...
    if (mode === "walking") {
      const WalkingRoute = (_AMap as any).WalkingRoute;
      if (!WalkingRoute) { error.value = "步行路线服务加载失败"; loading.value = false; return; }
      const walking = new WalkingRoute({ map: amap, autoFitView: true });
      walking.search(fromLL, toLL, (status: string, result: any) => {
        loading.value = false;
        if (status === "complete" && result.routes?.length) {
          processSingleRoute(result.routes[0]);
          renderMarkers(props.from!.lng, props.from!.lat, props.to!.lng, props.to!.lat);
        } else {
          error.value = "步行路线计算失败";
        }
      });
      return;
    }
    ...
    if (mode === "riding") {
      const RidingRoute = (_AMap as any).RidingRoute;
      if (!RidingRoute) { error.value = "骑行路线服务加载失败"; loading.value = false; return; }
      const riding = new RidingRoute({ map: amap, autoFitView: true });
      riding.search(fromLL, toLL, (status: string, result: any) => {
        loading.value = false;
        if (status === "complete" && result.routes?.length) {
          processSingleRoute(result.routes[0]);
          renderMarkers(props.from!.lng, props.from!.lat, props.to!.lng, props.to!.lat);
        } else {
          error.value = "骑行路线计算失败";
        }
      });
      return;
    }
    ...
    if (mode === "transit") {
      const TransferRoute = (_AMap as any).TransferRoute;
      if (!TransferRoute) { error.value = "公交路线服务加载失败"; loading.value = false; return; }
      const transit = new TransferRoute({ map: amap, city: props.from?.name || "", autoFitView: true });
      transit.search(fromLL, toLL, (status: string, result: any) => {
        loading.value = false;
        if (status === "complete" && result.routes?.length) {
          processTransitRoute(result.routes[0]);
          renderMarkers(props.from!.lng, props.from!.lat, props.to!.lng, props.to!.lat);
        } else {
          error.value = "公交路线计算失败，尝试切换为驾车模式";
        }
      });
      return;
    }
